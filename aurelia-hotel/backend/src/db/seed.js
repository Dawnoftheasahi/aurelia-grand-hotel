...
// All images verified from Unsplash – using stable photo IDs
const ROOMS = [
  {
...
function genCode() { return 'AUR' + Math.random().toString(36).substring(2, 8).toUpperCase(); }
...
async function seed() {
  ...
  const u1 = db.prepare('INSERT INTO users (name,email,password,phone,role) VALUES (?,?,?,?,?)').run('Alexandra Chen','alex@example.com',hp,'+1 555 0101','guest');
  const u2 = db.prepare('INSERT INTO users (name,email,password,phone,role) VALUES (?,?,?,?,?)').run('Marcus Webb','admin@aurelia.com',ap,'+1 555 0100','admin');
  const u3 = db.prepare('INSERT INTO users (name,email,password,phone,role) VALUES (?,?,?,?,?)').run('Sophie Laurent','sophie@example.com',hp,'+1 555 0102','guest');
  const u4 = db.prepare('INSERT INTO users (name,email,password,phone,role) VALUES (?,?,?,?,?)').run('James Whitfield','james@example.com',hp,'+1 555 0103','guest');
  console.log('✅ Users seeded');

  const ins = db.prepare('INSERT INTO rooms (name,type,description,price_per_night,capacity,bed_type,size_sqm,floor,view_type,images,amenities,rating,review_count) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)');
  const ids = [];
  for (const r of ROOMS) {
    const res = ins.run(r.name,r.type,r.description,r.price_per_night,r.capacity,r.bed_type,r.size_sqm,r.floor,r.view_type,r.images,r.amenities,r.rating,r.review_count);
    ids.push(res.lastInsertRowid);
  }
  console.log('✅ Rooms seeded');

  const ib = db.prepare('INSERT INTO bookings (user_id,room_id,check_in,check_out,guests,nights,room_price,extras,extras_price,total_price,status,special_requests,confirmation_code,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
  // Past completed bookings
  ib.run(u1.lastInsertRowid,ids[1],'2025-11-10','2025-11-14',2,4,180,JSON.stringify({breakfast:true}),60,780,'completed','Quiet room please',genCode(),'2025-10-20 10:00:00');
  ib.run(u1.lastInsertRowid,ids[4],'2026-01-20','2026-01-25',2,5,210,JSON.stringify({airportTransfer:true}),80,1130,'completed',null,genCode(),'2025-12-15 14:30:00');
  // Upcoming booking
  const f1 = new Date(); f1.setDate(f1.getDate()+14);
  const f2 = new Date(); f2.setDate(f2.getDate()+17);
  ib.run(u1.lastInsertRowid,ids[2],f1.toISOString().split('T')[0],f2.toISOString().split('T')[0],2,3,260,JSON.stringify({breakfast:true,lateCheckout:true}),80,860,'confirmed','Anniversary - flowers please',genCode(),new Date().toISOString());
  // Cancelled
  ib.run(u1.lastInsertRowid,ids[0],'2026-02-05','2026-02-07',1,2,120,'{}',0,240,'cancelled',null,genCode(),'2026-01-10 09:00:00');
  // Other users' bookings (for admin view)
  ib.run(u3.lastInsertRowid,ids[4],'2025-12-01','2025-12-05',2,4,210,JSON.stringify({breakfast:true}),200,1040,'completed',null,genCode(),'2025-11-15 08:00:00');
  ib.run(u3.lastInsertRowid,ids[5],'2026-03-10','2026-03-14',2,4,380,JSON.stringify({airportTransfer:true}),80,1600,'confirmed',null,genCode(),'2026-02-01 09:30:00');
  const f3 = new Date(); f3.setDate(f3.getDate()+30);
  const f4 = new Date(); f4.setDate(f4.getDate()+35);
  ib.run(u4.lastInsertRowid,ids[7],f3.toISOString().split('T')[0],f4.toISOString().split('T')[0],3,5,580,'{}',0,2900,'confirmed','VIP guest - prepare suite',genCode(),new Date().toISOString());
  console.log('✅ Bookings seeded');

  const ir = db.prepare('INSERT INTO reviews (user_id,room_id,rating,title,comment,created_at) VALUES (?,?,?,?,?,?)');
  ir.run(u1.lastInsertRowid,ids[1],5,'Exceptional stay','The Deluxe Room exceeded all expectations. Immaculate service and breathtaking views.','2025-11-15 10:00:00');
  ir.run(u3.lastInsertRowid,ids[1],4,'Wonderful experience','Beautiful room, very comfortable. Staff were incredibly attentive throughout our stay.','2025-12-20 14:00:00');
  ir.run(u3.lastInsertRowid,ids[4],5,'The ocean views are unreal','Waking up to those views every morning was magical. Absolutely worth every penny.','2026-01-10 09:00:00');
  ir.run(u2.lastInsertRowid,ids[5],5,'Beyond luxury','The Grand Suite is simply magnificent. Every detail was perfect.','2025-09-05 16:00:00');
  ir.run(u1.lastInsertRowid,ids[4],5,'Perfect romantic getaway','The balcony, the views, the service — everything was perfect for our anniversary.','2026-01-26 12:00:00');
  ir.run(u4.lastInsertRowid,ids[7],5,'Once in a lifetime experience','The Presidential Suite is in a league of its own. Private pool, butler service — extraordinary.','2025-08-10 11:00:00');
  console.log('✅ Reviews seeded');

  const ip = db.prepare('INSERT INTO promotions (code,title,description,discount_percent,is_active) VALUES (?,?,?,?,?)');
